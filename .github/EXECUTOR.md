...
When the issue touches code (not docs-only):

1. Read `plan.json` **reach** and **co_changed** fields — minimize blast radius.
2. Do not hand-edit `docs/modules/*.md` or `docs/radar/*.md` (generated by `docgen.py` / `radar_report.py`).
3. If you change scan/intel/graph tooling, ensure `python3 scripts/github_intel.py` would add `modifies` edges for your commits (verify runs `graph_delta.py`).
4. Prefer additive changes to `docs/index.json` schema (`schema_version` bumps only when required).
